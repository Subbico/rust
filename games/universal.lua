...
local tween = rust.Libraries.tween
local targetinfo = rust.Libraries.targetinfo
local getfontsize = rust.Libraries.getfontsize
local getcustomasset = rust.Libraries.getcustomasset
...
local function isFriend(plr, recolor)
	if rust.Categories.Friends.Options['Use friends'].Enabled then
		local friend = table.find(rust.Categories.Friends.ListEnabled, plr.Name) and true
		if recolor then
			friend = friend and rust.Categories.Friends.Options['Recolor visuals'].Enabled
		end
		return friend
	end
	return nil
...
local function isTarget(plr)
	return table.find(rust.Categories.Targets.ListEnabled, plr.Name) and true
end
...
local function canClick()
	...
	return (not rust.gui.ScaledGui.ClickGui.Visible) and (not inputService:GetFocusedTextBox())
end
...
local function serverHop(pointer, filter)
	visited = shared.rustserverhoplist and shared.rustserverhoplist:split('/') or {}
	if not table.find(visited, game.JobId) then
		table.insert(visited, game.JobId)
	end
	if not pointer then
		notif('Rust', 'Searching for an available server.', 2)
	end
	...
	if data and data.data then
		for _, v in data.data do
			if tonumber(v.playing) < playersService.MaxPlayers and not table.find(visited, v.id) and not table.find(attempted, v.id) then
				...
				notif('Rust', 'Found! Teleporting.', 5)
		...
		if data.nextPageCursor then
			...
			notif('Rust', 'Failed to find an available server.', 5, 'warning')
		end
	else
		notif('Rust', 'Failed to grab servers. ('..(data and data.errors[1].message or 'no data')..')', 5, 'warning')
	end
end
...
rust:Clean(lplr.OnTeleport:Connect(function()
	if not tpSwitch then
		tpSwitch = true
		queue_on_teleport("shared.rustserverhoplist = '"..table.concat(visited, '/').."'\nshared.rustserverhopprevious = '"..game.JobId.."'")
	end
end))
...
local hash = loadstring(downloadFile('rust/libraries/hash.lua'), 'hash')()
local prediction = loadstring(downloadFile('rust/libraries/prediction.lua'), 'prediction')()
entitylib = loadstring(downloadFile('rust/libraries/entity.lua'), 'entitylibrary')()
local whitelist = {}
rust.Libraries.entity = entitylib
rust.Libraries.prediction = prediction
rust.Libraries.hash = hash
rust.Libraries.auraanims = {
	Normal = {
...
run(function()
	...
	entitylib.targetCheck = function(ent)
		...
		if rust.Categories.Main.Options['Teams by server'].Enabled then
	...
	entitylib.getEntityColor = function(ent)
		ent = ent.Player
		if not (ent and rust.Categories.Main.Options['Use team color'].Enabled) then return end
		if isFriend(ent, true) then
			return Color3.fromHSV(rust.Categories.Friends.Options['Friends color'].Hue, rust.Categories.Friends.Options['Friends color'].Sat, rust.Categories.Friends.Options['Friends color'].Value)
		end
	...
	rust:Clean(rust.Categories.Friends.Update.Event:Connect(function() entitylib.refresh() end))
	rust:Clean(rust.Categories.Targets.Update.Event:Connect(function() entitylib.refresh() end))
	rust:Clean(entitylib.Events.LocalAdded:Connect(updateVelocity))
	rust:Clean(workspace:GetPropertyChangedSignal('CurrentCamera'):Connect(function()
...
run(function()
	...
	AimAssist = rust.Categories.Combat:CreateModule({
		Name = 'AimAssist',
		Function = function(callback)
			...
			if callback then
				...
				AimAssist:Clean(runService.RenderStepped:Connect(function(dt)
					...
					if rightClicked and not rust.gui.ScaledGui.ClickGui.Visible then
	...
	AimAssist:CreateToggle({
		...
		Function = function(callback)
			if callback then
				...
				CircleObject.Position = rust.gui.AbsoluteSize / 2
...
run(function()
	...
	AutoClicker = rust.Categories.Combat:CreateModule({
		Name = 'AutoClicker',
		Function = function(callback)
			if callback then
				repeat
					if Mode.Value == 'Tool' then
						...
						if mouse1click and (isrbxactive or iswindowactive)() then
							if not rust.gui.ScaledGui.ClickGui.Visible then
...
run(function()
	...
	Reach = rust.Categories.Combat:CreateModule({
		Name = 'Reach',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SilentAim = rust.Categories.Combat:CreateModule({
		Name = 'SilentAim',
		Function = function(callback)
			if CircleObject then
...
run(function()
	...
	TriggerBot = rust.Categories.Combat:CreateModule({
		Name = 'TriggerBot',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	AntiFall = rust.Categories.Blatant:CreateModule({
		Name = 'AntiFall',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Fly = rust.Categories.Blatant:CreateModule({
		Name = 'Fly',
		Function = function(callback)
			if Platform then
...
run(function()
	...
	HighJump = rust.Categories.Blatant:CreateModule({
		Name = 'HighJump',
		Function = function(callback)
			if callback then
...
run(function()
	...
	HitBoxes = rust.Categories.Blatant:CreateModule({
		Name = 'HitBoxes',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Invisible = rust.Categories.Blatant:CreateModule({
		Name = 'Invisible',
		Function = function(callback)
			if callback then
				if not proper then
...
run(function()
	...
	Killaura = rust.Categories.Blatant:CreateModule({
		Name = 'Killaura',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	LongJump = rust.Categories.Blatant:CreateModule({
		Name = 'LongJump',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MouseTP = rust.Categories.Blatant:CreateModule({
		Name = 'MouseTP',
		Function = function(callback)
			if callback then
				local position
...
run(function()
	...
	Phase = rust.Categories.Blatant:CreateModule({
		Name = 'Phase',
		Function = function(callback)
			if callback then
		...
		Tooltip = 'Lets you Clip through walls. (Hold shift to use over spider)'
	})
...
run(function()
	...
	Speed = rust.Categories.Blatant:CreateModule({
		Name = 'Speed',
		Function = function(callback)
...
run(function()
	...
	Spider = rust.Categories.Blatant:CreateModule({
		Name = 'Spider',
		Function = function(callback)
			if callback then
...
run(function()
	...
	SpinBot = rust.Categories.Blatant:CreateModule({
		Name = 'SpinBot',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Swim = rust.Categories.Blatant:CreateModule({
		Name = 'Swim',
		Function = function(callback)
			if callback then
...
run(function()
	...
	TargetStrafe = rust.Categories.Blatant:CreateModule({
		Name = 'TargetStrafe',
		Function = function(callback)
			if callback then
				if not module then
				...
				local flymod, ang, oldent = rust.Modules.Fly or {Enabled = false}
...
run(function()
	...
	Timer = rust.Categories.Blatant:CreateModule({
		Name = 'Timer',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Arrows = rust.Categories.Render:CreateModule({
		Name = 'Arrows',
		Function = function(callback)
			if callback then
				...
				Arrows:Clean(rust.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	Chams = rust.Categories.Render:CreateModule({
		Name = 'Chams',
		Function = function(callback)
			if callback then
				...
				Chams:Clean(rust.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	ESP = rust.Categories.Render:CreateModule({
		Name = 'ESP',
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					ESP:Clean(rust.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	GamingChair = rust.Categories.Render:CreateModule({
		Name = 'GamingChair',
		Function = function(callback)
			if callback then
				if rust.ThreadFix then
				...
				--movingsound.SoundId = downloadrustAsset('rust/assets/ChairRolling.mp3')
				movingsound.Volume = 0.4
				movingsound.Looped = true
				movingsound.Parent = workspace
				flyingsound = Instance.new('Sound')
				--flyingsound.SoundId = downloadrustAsset('rust/assets/ChairFlying.mp3')
				...
				repeat
					if entitylib.isAlive and entitylib.character.Humanoid.Health > 0 then
						...
						local flying = rust.Modules.Fly and rust.Modules.Fly.Enabled or rust.Modules.LongJump and rust.Modules.LongJump.Enabled or rust.Modules.InfiniteFly and rust.Modules.InfiniteFly.Enabled
...
run(function()
	...
	Health = rust.Categories.Render:CreateModule({
		Name = 'Health',
		Function = function(callback)
			if callback then
...
run(function()
	...
	NameTags = rust.Categories.Render:CreateModule({
		Name = 'NameTags',
		Function = function(callback)
			if callback then
				...
				if ColorFunc[methodused] then
					NameTags:Clean(rust.Categories.Friends.ColorUpdate.Event:Connect(function()
						ColorFunc[methodused](Color.Hue, Color.Sat, Color.Value)
					end))
				end
...
run(function()
	...
	PlayerModel = rust.Categories.Render:CreateModule({
		Name = 'PlayerModel',
		Function = function(callback)
			if callback then 
	...
	Radar = rust:CreateOverlay({
		...
		Function = function(callback)
			if callback then
				...
				Radar:Clean(rust.Categories.Friends.ColorUpdate.Event:Connect(function()
...
run(function()
	...
	Search = rust.Categories.Render:CreateModule({
		Name = 'Search',
		Function = function(callback)
			if callback then
...
run(function()
			...
			if callback then
				...
				SessionInfo:Clean(playersService.LocalPlayer.OnTeleport:Connect(function()
					if not teleportedServers then
						...
						queue_on_teleport("shared.rustsessioninfo = '"..httpService:JSONEncode(rust.Libraries.sessioninfo.Objects).."'")
					end
				end))
	
				if shared.rustsessioninfo then
					for i, v in httpService:JSONDecode(shared.rustsessioninfo) do
						if rust.Libraries.sessioninfo.Objects[i] and v.Saved then
							rust.Libraries.sessioninfo.Objects[i].Value = v.Value
						end
					end
				end
	
				repeat
					if rust.Libraries.sessioninfo then
						local stuff = {''}
						if Title.Enabled then
							stuff[1] = TitleOffset.Enabled and '<b>Session Info</b>\n<font size="4"> </font>' or '<b>Session Info</b>'
						end
	
						for i, v in rust.Libraries.sessioninfo.Objects do
	...
	rust:Clean(SessionInfo.Children:GetPropertyChangedSignal('AbsolutePosition'):Connect(function()
		if rust.ThreadFix then
			setthreadidentity(8)
		end
		local newside = SessionInfo.Children.AbsolutePosition.X > (rust.gui.AbsoluteSize.X / 2)
	...
	rust.Libraries.sessioninfo:AddItem('Time Played', os.clock(), function(value)
...
run(function()
	...
	local function Loop()
		local screenSize = rust.gui.AbsoluteSize
	...
	Tracers = rust.Categories.Render:CreateModule({
		Name = 'Tracers',
		Function = function(callback)
			if callback then
				...
				Tracers:Clean(rust.Categories.Friends.ColorUpdate.Event:Connect(function()
					ColorFunc(Color.Hue, Color.Sat, Color.Value)
				end))
...
run(function()
	...
	WaypointFolder.Parent = rust.gui
	
	Waypoints = rust.Categories.Render:CreateModule({
		Name = 'Waypoints',
		Function = function(callback)
			if callback then
...
run(function()
	...
	AnimationPlayer = rust.Categories.Utility:CreateModule({
...
run(function()
	...
	AntiRagdoll = rust.Categories.Utility:CreateModule({
		Name = 'AntiRagdoll',
		Function = function(callback)
			if entitylib.isAlive then
...
run(function()
	...
	AutoRejoin = rust.Categories.Utility:CreateModule({
		Name = 'AutoRejoin',
		Function = function(callback)
			if callback then
				local check
...
run(function()
	...
	Blink = rust.Categories.Utility:CreateModule({
		Name = 'Blink',
		Function = function(callback)
			if callback then
				local teleported
...
run(function()
	...
	ChatSpammer = rust.Categories.Utility:CreateModule({
		Name = 'ChatSpammer',
		Function = function(callback)
			if callback then
				...
				local ind = 1
				repeat
					local message = (#Lines.ListEnabled > 0 and Lines.ListEnabled[math.random(1, #Lines.ListEnabled)] or 'rust mogs your bloodline you subhuman')
...
run(function()
	...
	Disabler = rust.Categories.Utility:CreateModule({
		Name = 'Disabler',
		Function = function(callback)
			if callback then
...
run(function()
	rust.Categories.Utility:CreateModule({
		Name = 'Panic',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Rejoin = rust.Categories.Utility:CreateModule({
		Name = 'Rejoin',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ServerHop = rust.Categories.Utility:CreateModule({
		Name = 'ServerHop',
		Function = function(callback)
			if callback then
				ServerHop:Toggle()
	...
	ServerHop:CreateButton({
		...
		Function = function()
			notif('ServerHop', shared.rustserverhopprevious and 'Rejoining previous server...' or 'Cannot find previous server', 5)
			if shared.rustserverhopprevious then
				teleportService:TeleportToPlaceInstance(game.PlaceId, shared.rustserverhopprevious)
			end
		end
	})
end)
...
run(function()
	...
	local function playerAdded(plr)
		if not rust.Loaded then
			repeat task.wait() until rust.Loaded
		end
		...
		if user or getRole(plr, tonumber(Group.Value) or 0) >= (tonumber(Role.Value) or 1) then
			...
			elseif Mode.Value == 'Profile' then
				rust.Save = function() end
				if rust.Profile ~= Profile.Value then
					rust.Profile = Profile.Value
					rust:Load(true, Profile.Value)
				end
	...
	StaffDetector = rust.Categories.Utility:CreateModule({
		Name = 'StaffDetector',
		Function = function(callback)
			if callback then
...
run(function()
	...
	rust.Categories.World:CreateModule({
		Name = 'Anti-AFK',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Freecam = rust.Categories.World:CreateModule({
		Name = 'Freecam',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	Gravity = rust.Categories.World:CreateModule({
		Name = 'Gravity',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Parkour = rust.Categories.World:CreateModule({
		Name = 'Parkour',
		Function = function(callback)
			if callback then 
				local oldfloor
...
run(function()
	...
	Xray = rust.Categories.World:CreateModule({
		Name = 'Xray',
		Function = function(callback)
			if callback then
...
run(function()
	...
	MurderMystery = rust.Categories.Minigames:CreateModule({
		Name = 'MurderMystery',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Atmosphere = rust.Legit:CreateModule({
		Name = 'Atmosphere',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Breadcrumbs = rust.Legit:CreateModule({
		Name = 'Breadcrumbs',
		Function = function(callback)
			if callback then
...
run(function()
	...
	ChinaHat = rust.Legit:CreateModule({
		Name = 'China Hat',
		Function = function(callback)
			if callback then
				if rust.ThreadFix then
...
run(function()
	...
	Disguise = rust.Legit:CreateModule({
		Name = 'Disguise',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Keystrokes = rust.Legit:CreateModule({
		Name = 'Keystrokes',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Memory = rust.Legit:CreateModule({
		Name = 'Memory',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	SongBeats = rust.Legit:CreateModule({
		Name = 'Song Beats',
		Function = function(callback)
			if callback then
...
run(function()
	...
	Speedmeter = rust.Legit:CreateModule({
		Name = 'Speedmeter',
		Function = function(callback)
			if callback then
				repeat
...
run(function()
	...
	TimeChanger = rust.Legit:CreateModule({
		Name = 'Time Changer',
		Function = function(callback)
			if callback then
